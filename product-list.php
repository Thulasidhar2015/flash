...
if(count($products)>0) {
	...
	<!------------------------------------------------------------------------------------------------------------------------------------------------->	
		<div class="divbox" onclick="tb_show('<?=$pro['Title']?>','product-details.php?id=<?=$pro['ProductID']?>&keepThis=true&TB_iframe=true&height=500&width=900','thickbox');">
        	...
        	<div style="width: 202px;height: 250px;background-size:100%;background:url(images/products/medium/mobiles/<?=str_replace("_R","",$productPhoto)?>) no-repeat;" alt="<?=$pro['Title']?>">

...
if(!empty($scheck)) {
	...
	$inner = 'INNER JOIN tbl_products AS t2 ON t1.ProductID = t2.ProductID';
}
	$w = implode(' AND ',$WHERE);
	...
	if(mysql_num_rows($query)>0) {
		...
		<!------------------------------------------------------------------------------------------------------------------------------------------------->	
			<div class="divbox" onclick="tb_show('<?=$pro['Title']?>','product-details.php?id=<?=$pro['ProductID']?>&keepThis=true&TB_iframe=true&height=500&width=900','thickbox');">
				...
				<div style="width: 202px;height: 186px;background:url(images/products/medium/mobiles/<?=str_replace("_R","",$productPhoto)?>) no-repeat;" alt="<?=$pro['Title']?>">
